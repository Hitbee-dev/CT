...
2. 시험관의 개수만큼 반복 진행
3. 만약, 뺀 값이 0보다 작은 -값일 수도 있기 때문에 0보다 클때만 반복 진행
4. 시험관의 개수만큼 총 감독관의 수가 필요하므로 count에 더해줌
5. 각 시험관에 남은 학생 수 만큼 나눠서 몫과 나머지를 구함
6. 몫은 그대로 count에 더해주고(감독에 필요한 부 감독관의 수)
7. 나머지가 0이 아니라면 부 감독관이 1명 더 필요하니 count += 1
'''
